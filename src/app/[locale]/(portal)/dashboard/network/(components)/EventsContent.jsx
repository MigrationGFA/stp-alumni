      ...
      <CardContent>
        <div className="space-y-4">
          {upcomingEvents.length > 0 ? upcomingEvents.map((event) => {
            ...
            const hasSecureExternalLink = isOnline && event.externalLink && event.externalLink.startsWith('https');
            const href = hasSecureExternalLink ? event.externalLink : `/dashboard/events/${event.eventId}`;
            const target = hasSecureExternalLink ? "_blank" : "_self";
            const rel = hasSecureExternalLink ? "noreferrer noopener" : undefined;
            const actionLabel = hasSecureExternalLink ? 'Go to Link' : 'View Details';

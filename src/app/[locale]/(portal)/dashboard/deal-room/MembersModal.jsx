...
import { ShieldCheck, ShieldAlert, Users, Search, X, Loader2, UserPlus } from 'lucide-react';
...
export default function MembersModal({ open, onOpenChange, room, onAddMember, onRemoveMember }) {
  ...
  const availableConnections = useMemo(() => {
    return allConnections.filter(conn => !existingMemberIds.has(conn.connectionId));
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Add or remove members from this deal room
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-3">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {search.trim().length < 2 && search.trim().length > 0 && "Type at least 2 characters to search"}
                {search.trim().length === 0 && "Search your connections to add them to this room"}
              </p>
            </div>
                          ...
                          >
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                              <Avatar className="h-9 w-9 shrink-0">
                                <AvatarImage src={connection.profileImagePath || '/assets/Profile Image.jpg'} />
                                <AvatarFallback>
            ...
            {/* Empty search state */}
            {search.trim().length === 0 && availableConnections.length > 0 && (
              <div className="bg-blue-50 rounded-lg p-3">
                <p className="text-xs text-blue-800">
                  💡 Search for connections by name, company, or title to add them to this room.
                </p>
              </div>
            )}
          </div>
                  ...
                  {members.map((member) => (
                    ...
                    >
                      <Avatar className="h-8 w-8 shrink-0">
                        <AvatarImage src={member.avatar || member.profileImagePath} />
            ...
            {members.length > 0 && (
              <div className="bg-gray-50 rounded-lg p-2.5 border border-gray-100">
                <p className="text-xs text-muted-foreground flex items-center gap-2">
                  ...
                  <span>Members marked with <ShieldCheck className="h-3 w-3 inline text-green-500" /> have signed the NDA</span>
                </p>
              </div>
            )}
          </div>
